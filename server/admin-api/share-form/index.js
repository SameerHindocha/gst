...
var sendSMS = function get(req, res) {
  ...
  user.findOne({ email: email }, function(err, user) {
    if (err) {
      ...
      request('http://enterprise.smsgupshup.com/GatewayAPI/rest?method=SendMessage&send_to=' + mobile + '&msg=' + msg + '&msg_type=TEXT&userid=${global.config.gupShupSMS.userId}&auth_scheme=plain&password=${global.config.gupShupSMS.password}&v=1.1&format=text', function(error, response, body) {
        if (error) {
